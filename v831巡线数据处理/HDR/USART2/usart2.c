  ...
  u8 res; // 接受的临时变量
    ...
    //0x2C帧头
		...
		{  
				...
				if(RxCounter1 > 6)  //RxBuffer1接受满了,接收数据结束
				{	
           RxState=0;
//           if(RxBuffer1[6] == 0x6B)//帧尾		
//           {						 
					 // 2个字节转换 
           X = RXdeta(RxBuffer1[0],RxBuffer1[1]); 
           Y = RXdeta(RxBuffer1[2],RxBuffer1[3]);
					//坐标
           rotation = RXdeta(RxBuffer1[4],RxBuffer1[5]);//倾斜角			
					 printf("%d %d %d\r\n",X,Y,rotation);
//					 }
				}	  	
		}
